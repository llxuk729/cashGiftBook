  ...
  <Layout>
    <div class="add-record-page">
      ...
      <van-popup v-model:show="showRelationshipPicker" position="bottom" round>
        <van-picker :columns="relationshipColumns" @confirm="onRelationshipConfirm"
...
const selectedRelationshipName = computed(() => {
  ...
  return person ? `${person.name}（${getRelationshipText(person.relationship)}）` : '';
});
...
function onRelationshipConfirm({ selectedOptions }) {
  formData.value.relationshipId = selectedOptions[0].value;
